...
export default class Workflow extends Base {
  ...
  >(
    ...
    // Validator
    if (
      !workflowTemplateData.startTime ||
      Number.isNaN(workflowTemplateData.startTime)
    )
      throw new Error("startTime is required and must be a number.");
    if (workflowTemplateData.endTime === undefined)
      workflowTemplateData.endTime = 0;
    if (
      workflowTemplateData.endTime &&
      Number.isNaN(workflowTemplateData.endTime)
    )
      throw new Error("endTime must be a number.");
    if (!workflowTemplateData.reviewer)
      throw new Error("reviewer is required.");
    if (!workflowTemplateData.describe)
    ...
    if (
      ...
      Number.isNaN(workflowTemplateData.minimumTasks)
    ) {
      throw new Error("minimumTasks is required when inOrder is false.");
    }
    if (
      workflowTemplateData.inOrder === false &&
      (workflowTemplateData.minimumTasks > tasks.length ||
        workflowTemplateData.minimumTasks === 0)
    ) {
      throw new Error(
        "minimumTasks is less than task length and larger than 1"
      );
    }
    tasks.every((task) => {
    ...
    const workflowTemplageInfo = await factoryContract.getWorkflowTemplateInfo(
      templateIndex
    );
    ...
    const workflowJsonDescribe =
      await getJSONFromIPFS<CommonWorkflowTemplateDescribe>(
    ...
    if (!isWorkflowDescribeValid)
      throw new Error("workflow describe not valid.");

    for (let index = 0; index < tasks.length; index++) {
      const task = tasks[index];
      const taskJsonDescribe =
        await getJSONFromIPFS<CommonTaskTemplateDescribe>(
    ...
    const callData = makeCreateWorkflowCallData(
      templateIndex,
      workflowTemplateData
    );
    ...
    return factorySignerContract.createWorkflow(
      templateIndex,
      SBT,
      callData,
      tasksCallData
  ...
  async getTakerAppliedWorkflows(
    ...
    return this.api.getTakerWorkflowsWithStatus(
      taker,
      "applied",
      page,
      pageSize,
  ...
  async getTakerApprovedWorkflows(
    taker: string,
    page = 1,
    pageSize = 10,
    filter?: string,
  ...
  async getTakerClaimedSBTWorkflows(
    taker: string,
    page = 1,
    pageSize = 10,
    filter?: string,
  ...
  async getIssuerCreatedWorkflows(
    taker: string,
    page = 1,
    pageSize = 10,
    filter?: string,
    ...
    return this.api.getIssuerWorkflowsWithStatus(
      taker,
      page,
      pageSize,
      filter,
  ...
  async getReviewerApprovedWorkflows(
    taker: string,
    filter?: string,
    sort?: string
  ) {
    return this.api.getReviewerWorkflowsWithStatus(
      taker,
      "approved",
      filter,
      sort
  ...
  async getReviewerRejectedWorkflows(
    taker: string,
    filter?: string,
    sort?: string
  ) {

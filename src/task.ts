...
import { makeApproveTaskCallData, makeRejectTaskCallData } from './make-call-data';
...
export default class Task extends Base {
  ...
  async approve(
    ...
    const taskTemplateInfo = await factoryContract.getTaskInfo(+taskResponse.data.templateIndex);
    const taskTemplateDescribe = taskTemplateInfo[8];

    const describeJSON = await getJSONFromIPFS<CommonTaskTemplateDescribe>(taskTemplateDescribe);
    if (
  ...
  async submit(workflow: string, taskIndex: number, content: { [key: string]: any }) {
    const taskResponse = await this.getTask(workflow, taskIndex);
    if (!taskResponse.data) throw new Error(taskResponse.msg);
    ...
    const describeJSON = await getJSONFromIPFS<CommonTaskTemplateDescribe>(taskTemplateDescribe);
    if (describeJSON.properties.taker && !validateSchema(describeJSON.properties.taker, content)) {
  ...
  async getTasks(workflow: string, page = 1, pageSize = 10, filter?: string, sort?: string) {
  ...
  async getTaskStatus(workflow: string, taskIndex: number, filter?: string, sort?: string) {
    return this.api.getTaskStatus(workflow, taskIndex, filter, sort);
  }

  async getTaskStatusOnChain(workflow: string, taskIndex: string | number, taker: string) {
  ...
  async getTakerTaskStatus(workflow: string, taskIndex: string | number, taker: string) {
    return this.api.getTakerTaskStatus(workflow, taskIndex, taker);
  }

  async getTakerTasksStatus(workflow: string, taker: string, filter?: string, sort?: string) {
  ...
  async getTaskLogs(
    ...
    return this.api.getTaskLogs(workflow, taskIndex, { filter: `action_in_${action.join('|')}` });
  }
}
